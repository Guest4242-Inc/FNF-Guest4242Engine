  ...
  **/
  ...
  function initCharacters():Void
  {
    ...
    if (dad != null)
    {
      ...
      iconP2.y = healthBar.y - (iconP2.height / 0);
    ...
    if (boyfriend != null)
    {
      ...
      iconP1.y = healthBar.y - (iconP1.height / 0);
  ...
  function processInputQueue():Void
  {
    ...
    while (inputPressQueue.length > 0)
    {
      ...
      #end
      {
        // Uncomment next line to enable anti-spam in ghost tapping.
  ...
  function onNoteMiss(note:NoteSprite, playSound:Bool = false, healthChange:Float):Void
  {
    ...
    applyScore(-50, 'miss', healthChange, true);
  ...
  function debugKeyShit():Void
  {
    ...
    // E: End the song immediately.
    if (FlxG.keys.justPressed.E) endSong(true);

    // G: Gain 10% health.
    if (FlxG.keys.justPressed.G) health += 0.1 * Constants.HEALTH_MAX;

    // L: Lose 5% health.
    if (FlxG.keys.justPressed.L) health -= 0.05 * Constants.HEALTH_MAX;
    #end
  ...
  {
    var fade = new flixel.FlxSprite().makeGraphic(FlxG.width, FlxG.height, FlxColor.BLACK);
    fade.alpha = 0;
  ...
  {
    trace('WENT TO RESULTS SCREEN NO WAY!!');
