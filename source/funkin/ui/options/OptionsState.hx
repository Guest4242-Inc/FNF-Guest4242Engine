...
class OptionsMenu extends Page
{
  ...
  public function new()
  {
    ...
    createItem("PREFERENCES MENU", function() switchPage(Preferences));
    createItem("KEYBOARD SHORTCUTS", function() switchPage(Controls));
    ...
    #if newgrounds
    if (NGio.isLoggedIn) createItem("LOGOUT FROM NEWGROUNDS", selectLogout);
    else
      createItem("LOGIN TO NEWGROUNDS", selectLogin);
    #end
