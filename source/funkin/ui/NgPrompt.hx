...
class NgPrompt extends Prompt
{
    ...
    prompt.openCallback = NGio.login.bind(function popupLauncher(openPassportUrl)
    {
      ...
      if (choiceMsg != null)
      {
        ...
        prompt.onYes = function() {
          prompt.setText("Connecting to Newgrounds server..." #if web + "\n(check your popup blocker)" #end);
      ...
      {
        prompt.setText("Connecting to Newgrounds server via browser...");
        openPassportUrl();
      }
        ...
        case Fail(msg):
          {
            ...
            prompt.setText("Login failed, please try again.");
            prompt.setButtons(Ok);
            prompt.onYes = prompt.close;
          }
        case Cancelled:
          {
            if (prompt != null)
            {
              prompt.setText("Login was manually cancelled by the user");
              prompt.setButtons(Ok);
              prompt.onYes = prompt.close;
            }
            else
              trace("Login was manually cancelled by the user");
          }
      }
    });
  ...
  static public function showLogout()
  {
    ...
    var prompt = new NgPrompt('Log out of $user? Some data may be lost.', Yes_No);
    prompt.onYes = function() {
      NGio.logout();
      prompt.close();
    };
