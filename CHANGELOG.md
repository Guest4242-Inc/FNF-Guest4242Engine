...
### Changed
- Changed the botplay text in the playstate.
- Added combo breaks and combo to the playstate.
- Updated the engine to Friday Night Funkin' v0.5.3.
- Fixed the Polymod support a bit, so mods should work better (hopefully).
- Engine is almost compatible with Kade Engine's mods (many of them does not work, but only 2 of them works).
- Changed the engine watermark text in the main menu.
- Remade the HUD a bit. (0.0.1 was using fancy hud by ralsi)
### Added
- Some new hidden features that are only available in a hidden debug menu.
  - Press 9 to open it.
  - Note that debug menu is available only for special people.
- Some other stuff I can't describe here.
  - If you want to know, here's one of them:
    - Flashlight: There is a litte chance that flashlight will be enabled unexpectedly, and you light up the stage using your mouse.
### Notes
- This version is still in beta testing.
- This version development is P A I N F U L, and I am still bad at Haxe. Code is still spaghetti.
- Some code is written by Claude 3.5 Sonnet, so it could be trash, unstable, buggy and etc.
- Many of Kade Engine mods does not work, I'll fix that someday.

## [0.0.1] - 2024-11-04
### Added
- Made a new engine called Guest4242 Engine.
- New HUD
- Etc.
### Notes
- Only works on Windows. Should work fine on Wine, but some compatibility issues may occur.
- May have bugs, errors, and crashes that I am not sure how to fix.
- Some mods may not work.
- This is my first time coding in Haxe, so the code is probably spaghetti.
- The engine is based on vanilla Friday Night Funkin', so it may have some bugs that are in vanilla. (some of them are fixed, some are not)
- This version has some bugs in the camera, I'll try to fix them someday.
- This engine is fully compatible with Polymod, so you can use any mods you want. (not all of them will work through in this version, but most of them should)
- The engine was compiled using Haxe 4.3.6, so if you are using an older version of Haxe, you may get some errors.
